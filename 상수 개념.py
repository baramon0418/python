...
#1.상수란 무엇인가?
#항상 변하지 않는 수--> 상수
#코드를 작성하는 과정에서 오직 하나의 데이터값을 많이 사용하는데 이런 데이터 값을 담는 바구니--> 상수
#요리에 비유하면 오직 한가지 양념을 담는 양념통--> 상수
...
#따라서 변수처럼 상수 변수를 선언하고 사용은 할 수 있겠지만, 강제로 변경하고자 한다면 변경이 가능하다. --> 변수
...
#[2]: 변수를 상수처럼 사용
#위의 PI,GRAVITY 변수를 대문자로 선언하고 값을 할당함으로써 상수처럼 사용하겠다고 정의한다.
print(PI)
PI = 4.14 #---어? 변경이 되네..??? --> 변수--;;
